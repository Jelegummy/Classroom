...
export const createGameSession = async (args: CreateGameArgs) => {
    ...
    const res = await fetchers.Post<{ accessToken: string }>(`${ENDPOINT}/game/internal/create`, {
        data: args,
...
export const addItemToGame = async (gameId: string, itemId: string) => {
    ...
    const res = await fetchers.Post<{ accessToken: string }>(`${ENDPOINT}/game/internal/Item/item-in-game/${gameId}/${itemId}`, {
        token: session?.user.accessToken,
    })
