...
export const createCharacter = async (args: CreateCharacter) => {
  ...
  const res = await fetchers.Post<{ accessToken: string }>(
    `${ENDPOINT}/character/internal/create/character`,
    {
      data: args,
...
export const getAllCharacters = async () => {
  ...
  const res = await fetchers.Get<Character[]>(
    `${ENDPOINT}/character/internal/all`,
    {
      token: session?.user.accessToken,
    },
  )
...
export const getCharacter = async (id: string) => {
  ...
  const res = await fetchers.Get<Character>(
    `${ENDPOINT}/character/internal/character/${id}`,
    {
      token: session?.user.accessToken,
    },
  )
