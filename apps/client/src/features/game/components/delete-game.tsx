import { deleteGameSession } from '@/services/game-session'
...
export default function DeleteGame({ id, name }: DeleteGameProps) {
  ...
  const deleteTutorMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('ลบห้องเล่นเกมสำเร็จ')
      setOpen(false)
      setConfirmId('')
      queryClient.invalidateQueries({ queryKey: ['getAllGameSessions'] })
    },
    onError: e => {
