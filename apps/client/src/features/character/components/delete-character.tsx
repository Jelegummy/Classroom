...
import { DeleteCharacterProps } from '../types'
import { deleteCharacter } from '@/services/charecter'
...
export default function DeleteCharacter({
  ...
  const isMatch = confirmId === bossName
  const onDelete = () => {
    if (isMatch) {
  ...
  const handleCopy = (e?: React.MouseEvent) => {
    ...
    navigator.clipboard.writeText(bossName)
    setIsCopied(true)
        ...
        <dialog className="modal modal-open bg-black/40 backdrop-blur-sm">
          <div className="modal-box relative w-full max-w-md rounded-2xl bg-white p-7 shadow-2xl">
            ...
            <p className="mb-4 text-sm text-gray-600">
              ...
              <span className="font-semibold text-gray-900">{bossName}</span>{' '}
              ด้านล่าง
            </p>
            ...
            <div className="mb-6">
              <input
                ...
                placeholder={`พิมพ์ "${bossName}" เพื่อยืนยัน...`}
