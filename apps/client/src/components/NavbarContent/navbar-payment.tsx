...
export default function NavbarPayment({ UserId }: NavbarPaymentProps) {
  const { data: session } = useSession()
  const { data: pointsUser } = useQuery({
    queryKey: ['getUserPoint', UserId],
        ...
        <div className="sticky top-0 flex h-20 items-center border-b bg-white px-8">
          <div className="flex w-full items-center justify-end gap-3">
            <div className="flex flex-row items-center gap-2 rounded-lg bg-[#EA9000] p-2 px-4 py-1 text-white">
              ...
              <h1 className="font-bold">{pointsUser?.points}</h1>
            </div>
