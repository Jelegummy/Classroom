...
export class TutorPublicService {
    ...
    async createTutorBot(id: string, args: CreateTutorVoiceLogArgs, botSecret?: string) {
        ...
        const content = typeof args.dataContent === 'string'
        ...
        if (roles) {
            ...
            if (activeTutorSession) {
                ...
                const prepareScoreQueries = async (fullName: string, pointsEarned: number) => {
                    ...
                    if (!user) {
                        throw new NotFoundException(`User with name "${fullName}" not found`);
                    }
